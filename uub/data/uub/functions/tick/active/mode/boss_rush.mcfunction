...
execute if score #timeout pseudo_health matches ..0 run scoreboard players set #timeout pseudo_health 3
...
execute as @a[team=boss,scores={pseudo_health=..0}] run function uub:death/boss
execute as @a[team=boss,scores={bool.die=1..}] run function uub:death/boss
...
scoreboard players operation @a[team=fighters] display_health = @r[team=fighters] health
