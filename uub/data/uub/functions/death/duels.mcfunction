...
tag @a[tag=killer,scores={bool.die=1..}] remove alive

execute unless entity @a[tag=alive] run tellraw @a {"text": "Tie!","color": "gold"}
execute if entity @a[tag=alive] run tellraw @a [{"selector": "@a[tag=killer]","color": "green"},{"text": " wins! ","color": "green"},{"text": "[Details]","color": "dark_gray","hoverEvent": {"action": "show_text","contents": [{"selector": "@a[tag=alive]"},{"text": " was at ","color": "gray"},{"score": {"name": "@r[tag=alive]","objective": "health"}},{"text": "HP.","color": "dark_red"}]}}]
