...
# Automatically ready players based on their settings
    execute if score .ready_timeout _var matches 1.. as @a[tag=player, tag=!ready] run function ult:start/ready/auto_test

# Repeat timer if not done counting down
    execute if score .ready_timeout _var matches 1.. run schedule function ult:start/ready/timer 1s

# End the timer if at zero
    execute if score .ready_timeout _var matches 0 run function ult:start/ready/timeout
