...
execute as @a[tag=killer,scores={die=1..}] run function ult:death/proc

execute unless entity @a[tag=alive] run tellraw @a {"text": "Tie!","color": "gold"}
execute unless entity @a[tag=alive] as @a[gamemode=!spectator, scores={die=1..}] run function ult:death/proc
