...
scoreboard players operation .faces _var += .dat _var
scoreboard players remove .faces _var 1
...
execute store result score .dat _var run data get storage ult:temp Game.brawl.freshkills
scoreboard players operation .freshkills _var += .dat _var
execute store result score .dat _var run data get storage ult:temp Game.brawl.normkills
scoreboard players operation .normkills _var += .dat _var
execute store result score .dat _var run data get storage ult:temp Game.brawl.revenge
scoreboard players operation .revenge _var += .dat _var
execute store result score .mode _var run data get storage ult:temp Game.mode
execute if score .mode _var = flag.game_mode.duels const run scoreboard players add .duels _var 1
execute if score .mode _var = flag.game_mode.royale const run scoreboard players add .royale _var 1
execute if score .mode _var = flag.game_mode.brawl const run scoreboard players add .brawl _var 1
