# Make sure the selected map does not match the current one
    scoreboard players operation temp.map var = .map flag
    scoreboard players operation .map flag = .selected_map var
    scoreboard players operation .map_mode flag = flag.map_mode.selected const
    execute unless score temp.map var = .map flag run function ult:settings/map/change
