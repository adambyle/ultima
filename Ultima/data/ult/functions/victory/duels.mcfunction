...
execute unless score .map_mode flag = flag.map_mode.selected const run title @a[tag=!parkour] title {"storage": "ult:temp", "nbt": "Map", "color": "green"}
execute unless score .map_mode flag = flag.map_mode.selected const run title @a[tag=parkour, tag=player] title {"storage": "ult:temp", "nbt": "Map", "color": "green"}
