...
execute if score .map event matches ..0 at @e[sort=random, limit=1, tag=dungeon_potion] run function ult:tick/maps/dungeon/spawn_potion

execute at @e[tag=dungeon_potion] as @e[type=falling_block, distance=..2] if entity @a[tag=player, tag=player, tag=alive, scores={altitude=21}, distance=..5] run function ult:tick/maps/dungeon/fill_potion
