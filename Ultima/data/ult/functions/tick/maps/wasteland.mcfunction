# With flint selected (in either hand) and crouching, use a smoke bomb
    execute as @a[scores={crouch_mode=1}, nbt={SelectedItem: {id: "minecraft:flint"}}] at @s run function ult:tick/maps/wasteland/smoke_bomb
    execute as @a[scores={crouch_mode=1}, nbt={Inventory: [{id: "minecraft:flint", Slot: -106b}]}] at @s run function ult:tick/maps/wasteland/smoke_bomb
...
# Give players who killed a rabbit a cooked rabbit meat item
    execute as @a[scores={event.rabbit=1..}, nbt=!{Inventory: [{id: "minecraft:cooked_rabbit"}]}] run function ult:tick/maps/wasteland/food

# With meat selected (in either hadn) and crouching, eat the food
    execute as @a[scores={crouch_mode=1}, nbt={SelectedItem: {id: "minecraft:cooked_rabbit"}}] run function ult:tick/maps/wasteland/eat_food
    execute as @a[scores={crouch_mode=1}, nbt={Inventory: [{id: "minecraft:cooked_rabbit", Slot: -106b}]}] run function ult:tick/maps/wasteland/eat_food
