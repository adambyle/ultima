# Deal with current netherite armor owners
    ...
    execute as @a[tag=player, tag=alive, scores={event=0}] run function ult:tick/maps/abyss/revert

# Allow players to equip
    # Players can use the netherite from either hand
    execute as @a[tag=player, tag=alive, scores={crouch_mode=1}, nbt={SelectedItem: {id: "minecraft:netherite_ingot"}}] run function ult:tick/maps/abyss/trigger
    execute as @a[tag=player, tag=alive, scores={crouch_mode=1}, nbt={Inventory: [{id: "minecraft:netherite_ingot", Slot: -106b}]}] run function ult:tick/maps/abyss/trigger
