# Respawn players when there is room
    ...
    execute as @a[tag=alive] run scoreboard players add .temp_pn var 1
    # Include players who are currently respawning
    execute as @a[tag=player, scores={respawn=0..}] run scoreboard players add .temp_pn var 1
    # If there is room, respawn the next player in line
    execute if score .temp_pn var matches ..3 as @r[tag=player, tag=!alive, scores={queue=1, respawn=-1}] run function ult:respawn
