# Respawn players when there is room
    ...
    execute if score .temp_pn _var matches ..3 as @r[tag=!alive, scores={queue=1, respawn=-1}] run function ult:respawn
...
# Tell dead players who they're tethered to
    execute as @a[tag=player, scores={tether=1..}] run function ult:tech/tether_notif
