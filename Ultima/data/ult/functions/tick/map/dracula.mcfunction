...
# Let any player pick up the trident (also simulated)
    execute as @e[type=trident, nbt={inGround: true, Trident: {tag: {Dead: true}}}] at @s if entity @a[tag=player, tag=alive, distance=..2] run function ult:tick/map/dracula/give
