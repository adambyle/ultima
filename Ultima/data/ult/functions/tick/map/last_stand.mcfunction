execute as @a[scores={crouch_mode=1}, nbt={SelectedItem: {id: "minecraft:coal"}}] at @s anchored eyes run function ult:tick/map/last_stand/fireball

tag @e remove exploded
execute as @e[type=fireball] at @s if entity @e[type=fireball, distance=0.01..5] unless entity @e[tag=text_display, distance=..20] run function ult:tick/map/last_stand/explode
execute as @e[type=fireball] at @s run tp @e[sort=nearest, limit=1, tag=fireball_tracker, distance=..3] @s
execute as @e[tag=fireball_tracker] at @s unless entity @e[type=fireball, distance=..1] unless entity @e[tag=text_display, distance=..20] run function ult:tick/map/last_stand/explode
