...
# Tell players what rune they have applied
    ...
    execute as @a[tag=alive] if score @s event.rune = event.rune.speed const run title @s actionbar {"text": "Δ Rune Applied -- Speed Up", "bold": true, "color": "gold"}
    execute as @a[tag=alive] if score @s event.rune = event.rune.health const run title @s actionbar {"text": "Π Rune Applied -- Health Up", "bold": true, "color": "red"}
    execute as @a[tag=alive] if score @s event.rune = event.rune.attack const run title @s actionbar {"text": "Σ Rune Applied -- Attack Up", "bold": true, "color": "aqua"}
    execute as @a[tag=alive] if score @s event.rune = event.rune.defense const run title @s actionbar {"text": "Γ Rune Applied -- Defense Up", "bold": true, "color": "green"}
