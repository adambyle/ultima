# Summon arrows for nearby players at the arrow refill station
    ...
    execute if score .map event matches ..0 run summon item 46 10 42 {Item: {Count: 1b, id: "arrow"}, Tags: ["static", "woodlands_arrow"]}
    ...
    execute positioned 46 10 42 unless entity @a[gamemode=adventure, tag=player, distance=..6] run kill @e[type=item, tag=woodlands_arrow]
