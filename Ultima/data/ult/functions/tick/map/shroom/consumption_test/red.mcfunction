# Test for enchanting capability
    ...
    execute if data entity @s Inventory[{id: "minecraft:bow", tag: {Enchantments: [{lvl: 3s}]}}] run title @s actionbar {"text": "Your bow is maxed.", "color": "gray"}
    execute if data entity @s Inventory[{id: "minecraft:iron_sword"}].tag.Enchantments run title @s actionbar {"text": "You already have Sharpness; you cannot get Power.", "color": "gray"}
