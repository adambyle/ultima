...
execute if score #map event matches 0 run function ult:tick/active/map/dracula/summon
...
execute as @e[type=trident, nbt={DealtDamage: true}, nbt=!{Trident: {tag: {Dead: true}}}] at @s run function ult:tick/active/map/dracula/lightning
execute as @e[type=trident, nbt={inGround: true, Trident: {tag: {Dead: true}}}] at @s if entity @a[tag=player, tag=alive, distance=..2] run function ult:tick/active/map/dracula/give
