...
execute if score #map event matches ..0 run loot give @a[tag=player, tag=alive] loot ult:map_items/townsquare/emerald
execute if score #map event matches ..0 run scoreboard players set #map event 40
loot give @a[scores={x.damage=1..}] loot ult:map_items/townsquare/emerald

execute as @a[scores={x.crouch=1}, nbt={SelectedItem: {id: "minecraft:netherite_ingot"}}] run function ult:event/townsquare/upgrade_armor
