...
execute if score #map event matches ..0 at @e[tag=dungeon_potion, limit=1, sort=random] run function ult:event/dungeon/spawn_potion

execute at @e[tag=dungeon_potion] as @e[type=falling_block, distance=..2] if entity @a[tag=player, scores={altitude=21}, tag=player, tag=alive, distance=..5] run function ult:event/dungeon/fill_potion
