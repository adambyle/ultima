execute as @a[scores={ancient.debris=1..}] run function ult:event/ancient/give_ingot
...
execute as @a[scores={_var=1..}] run function ult:event/ancient/upgrade

execute as @a[scores={x.crouch=1}, nbt={SelectedItem: {id: "minecraft:fire_charge"}}] at @s anchored eyes run function ult:event/ancient/fireball
scoreboard players reset * z.ancient.gold
scoreboard players reset * ancient.debris

tag @e remove exploded
execute as @e[type=fireball] at @s if entity @e[type=fireball, distance=0.01..5] run function ult:event/ancient/explode
execute as @e[type=fireball] at @s run tp @e[tag=fireball_tracker, sort=nearest, limit=1, distance=..3] @s
execute as @e[tag=fireball_tracker] at @s unless entity @e[type=fireball, distance=..1] run function ult:event/ancient/explode
