...
execute if predicate ult:maps/wasteland/rabbit_spawn at @r at @e[sort=random, limit=1, tag=wasteland_rabbit_spawn] unless entity @e[type=rabbit, distance=..12] run summon rabbit

execute as @a[scores={event.rabbit=1..}, nbt=!{Inventory: [{id: "minecraft:cooked_rabbit"}]}] run function ult:tick/active/map/wasteland/food
