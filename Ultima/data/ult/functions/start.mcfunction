...
execute if score .game_mode flag = #ultimate game_mode run scoreboard players set #server map 0
...
execute if score .game_mode flag = #ultimate game_mode run function ult:start/mode/ultimate
...
execute if score #flag map > #select map unless score .game_mode flag = #ultimate game_mode run title @a[tag=!parkour] title {"storage": "ult:temp", "nbt": "Map", "color": "green"}
execute if score #flag map > #select map unless score .game_mode flag = #ultimate game_mode run title @a[tag=parkour, tag=player] title {"storage": "ult:temp", "nbt": "Map", "color": "green"}
