# Find player at this point in queue
    ...
    execute as @a[tag=!alive] if score @s queue = .iter _var run tag @s add root
...
# Search for player next in queue
    ...
    execute as @a[tag=!alive] if score @s queue = .queue _var run scoreboard players set .can_deplete _var 0
