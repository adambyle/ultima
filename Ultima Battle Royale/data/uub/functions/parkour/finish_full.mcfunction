...
execute if score @s parkour matches 1 store result score #wr _var run data get storage uub:temp GlobalData.Full.Time
execute if score @s timer <= #wr _var run function uub:parkour/world_record
execute if score @s parkour matches 1 store result score #pb _var run data get storage uub:temp PlayerData.Full
...
execute if score @s timer > #wr _var if score @s timer >= #pb _var run tellraw @s [{"text": "You finished the full course in "}, {"storage": "uub:temp", "nbt": "Time", "color": "gray", "interpret": true}]
